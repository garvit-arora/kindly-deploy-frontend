...
function LogStream() {
    ...
    async (deploymentId) => {
      ...
      try {
        const response = await fetch(
          `${API_URL}/api/deployments/${deploymentId}/runtime-logs`,
          {
            credentials: "include",
          },
        );
  ...
  useEffect(() => {
    ...
    if (selectedDeployment?.status !== "READY") {
      return undefined;
    }

    const intervalId = window.setInterval(() => {
      loadLogs(selectedDeploymentId);
    }, 3000);

    return () => window.clearInterval(intervalId);
  }, [deployments, selectedDeploymentId, loadLogs]);
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-gray-400">
            Saved runtime output for a deployment.
          </p>
        </div>
      ...
      {selectedDeploymentId && hasRunningContainer && (
        <div className="mt-6 overflow-hidden rounded-xl border border-gray-800 bg-[#111]">
          ...
          <pre
            ...
            {logs || "No saved runtime logs yet."}
          </pre>
        </div>
      )}
